...
| Stage 2 — Session routing / approval wire correctness | open     | 3 (all Medium)       | Orphan frames, modal-without-deny, resolved-without-decide |
| Stage 3 — Display / terminal injection                | open     | 1 (Medium)           | Strip C0/C1/ESC before store/render/copy                   |
...
- [ ] **S2-F1** — Severity: Medium — Status: `open`
...
- [ ] **S2-F2** — Severity: Medium — Status: `open`
...
- [ ] **S2-F3** — Severity: Medium — Status: `open`
...
- [ ] **S3-F1** — Severity: Medium — Status: `open`
  - Files: `src/app.rs:27-36`; push paths in `src/provider.rs`, `src/codex.rs`, `src/grok.rs`, `src/agy.rs`; `src/ui.rs`
